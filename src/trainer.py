...
def train_loop(model, train_loader, val_loader, cfg, checkpoint_dir, trial=None): # Добавлен аргумент trial=None
    ...
    for epoch in range(cfg.train.epochs):
        ...
        # обновляем время ПЕРЕД evaluate() для правильного intermediate score
        if trial:
            avg_time_so_far = total_training_time / (epoch + 1)
            trial.set_user_attr('avg_time_per_epoch', avg_time_so_far)
    ...
    # ИСПРАВЛЕНИЕ: убираем возврат времени, оно теперь в trial.user_attrs
    ...
    # Финальное обновление времени для trial (если есть)
    if trial:
        trial.set_user_attr('avg_time_per_epoch', avg_time_per_epoch)
    
    return best_val_acc  # Возвращаем только accuracy
