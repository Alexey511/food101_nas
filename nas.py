...
def objective_architecture_search(trial, cfg):
    ...
    try:
        best_val_accuracy = train_loop(model, train_loader, val_loader, cfg, checkpoint_dir, trial)
    except RuntimeError as e:
        logger.error(f"Ошибка тренировки в Trial {trial.number}: {e}")
        raise optuna.TrialPruned()

    # ИСПРАВЛЕНИЕ: берем время из user_attrs (устанавливается в train_loop)
    avg_time_per_epoch = trial.user_attrs.get('avg_time_per_epoch', 0)
...
def objective_hyperparameter_search(trial, cfg, architecture_params):
    ...
    # Для гиперпараметров используем только accuracy
    # (архитектура уже выбрана с учетом efficiency)
    logger.info(f"Trial {trial.number} (hyperparameters) - Accuracy: {best_val_accuracy:.4f}, Params: {num_params:,}, Time per epoch: {avg_time_per_epoch:.2f}s")

    return best_val_accuracy

def select_best_model(study, cfg):
    """
    Выбирает лучшую модель по максимальному значению objective функции.
    """
    logger = logging.getLogger(__name__)
    best_trial = None
...
def optimize_process_priority():
    """Безопасно повышает приоритет процесса"""
    try:
        import psutil
        current_process = psutil.Process(os.getpid())
        
        if sys.platform == "win32":
            # Windows: устанавливаем высокий приоритет (безопасно)
            current_process.nice(psutil.HIGH_PRIORITY_CLASS)
            print(f"[OK] Установлен HIGH PRIORITY для NAS процесса (PID: {os.getpid()})")
        else:
            # Unix/Linux: устанавливаем высокий приоритет (безопасно)
            current_process.nice(-10)
            print(f"[OK] Установлен высокий приоритет для NAS процесса (PID: {os.getpid()})")
            
        print("TIP: Для дополнительной производительности:")
        print("   - Установите режим питания 'Высокая производительность'")
        print("   - Закройте ненужные приложения")
        print("   - Убедитесь в хорошем охлаждении системы")
            
    except ImportError:
        print("[WARNING] psutil не найден. Для лучшей производительности установите: pip install psutil")
    except Exception as e:
        print(f"[WARNING] Не удалось повысить приоритет: {e}")
        print("TIP: Попробуйте запустить от имени администратора")

def main(config_path):
    """Основная функция для запуска NAS."""
    # Безопасно повышаем приоритет процесса
    print("Оптимизация приоритета процесса...")
    ...
    try:
        ...
        # Используем `user_attrs['config']`, где лежат обработанные (совместимые) параметры
        best_architecture_params = best_trial_arch.user_attrs.get('config')
        if not best_architecture_params:
            logger.warning("Не удалось получить 'config' из user_attrs. Используем trial.params как fallback.")
    ...
    if best_hyp_trial:
        ...
        logger.info(f"Лучшие гиперпараметры: {best_hyperparameters} с accuracy {best_hyp_trial.value:.4f}")
        print("\nАнализ результатов поиска гиперпараметров:")
        print(f"Лучшие гиперпараметры: {best_hyperparameters}")
        print(f"   Accuracy: {best_hyp_trial.value:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="optuna_studies/nas_vit_custom_vit_config.yaml",
